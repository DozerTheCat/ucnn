...
//
// ============================================================================
//    train_cv.cpp: demonstrates configuring and training a new model
//
//    Instructions: 
...
void convert_cifar_color(std::vector<std::vector<float>> &train_images, std::vector<std::vector<float>> &test_images)
{

	for (int i = 0; i < train_images.size(); i++)
	{
		ucnn::matrix y = ucnn::bgr2ycrcb(ucnn::matrix(32, 32, 3, train_images[i].data()));
		memcpy(train_images[i].data(), y.x, sizeof(float)*y.size());
	}
	for (int i = 0; i < test_images.size(); i++)
	{
		ucnn::matrix y = ucnn::bgr2ycrcb(ucnn::matrix(32, 32, 3, test_images[i].data()));
		memcpy(test_images[i].data(), y.x, sizeof(float)*y.size());
	}
}
...
int main()
{
...
#ifndef USE_MNIST
	convert_cifar_color(test_images, train_images);
	...
	while(1)
	{
		...
		for(int k=0; k<train_samples; k++) 
		{
			...
			{
				//ucnn::show(m, 6, "input");
				ucnn::show(ucnn::draw_cnn_weights(cnn),6,"W");
